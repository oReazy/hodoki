...
async def addNewAccount(tg_id):  # Создание нового аккаунта в базе данных
    try:
        ...
        ROBOT_INFO = [0, 0]
        ROBOT_INFO_ARCHIVE = [[], [], []]
        QUESTIONLIST = ['Номер робота', 'Ветка']
        QUESTIONANSWERHISTORY = {}
        timestamp = int(time.time())
        connection = await connectBase()
        async with connection.cursor() as cursor:
            new_user = "INSERT INTO `users` (tg_id, tg_mainMessage, tg_answer, state, actions, logs, logs_archive, robot_info, settingTimeAdd, robot_info_archive, tg_lastMessage, tg_online, profile, temporary, settings_HodokForm, admin, tg_username, settings_help, sm, listQuestions, questionAnswerHistory) VALUES " \
                       f"('{tg_id}', " \
                       f"'-1', " \
                       f"'1', " \
...
async def getDataMulti(table, key, value):  # получение данных (выводит все)
    ...
    async with connection.cursor() as cursor:
        ...
        connection.close()
    return rows
async def getDataAllMulti(table):  # получение данных (выводит все)
    connection = await connectBase()
    async with connection.cursor() as cursor:
        select_row = f"SELECT * FROM `{table}`"
        await cursor.execute(select_row)
        rows = await cursor.fetchall()
        connection.close()
    return rows
...
# МАКРОСЫ
async def macrosMaps(sql, logs_value, tg_id):  # получение данных пользователя
    connection = await connectBase()
    async with connection.cursor() as cursor:
        query = sql
        await cursor.execute(query, (logs_value, tg_id))
        await connection.commit()
        connection.close()
